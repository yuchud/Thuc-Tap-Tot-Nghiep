...
describe('Account Controller Tests', () => {
  ...
  describe('Customer Account Tests', () => {
    ...
    test('Should return BAD_REQUEST when create account with duplicated email', async () => {
      await supertest(app).post('/api/accounts').expect(http.StatusCodes.BAD_REQUEST).send({
        username: 'yuchud1',
    ...
    test('Should delete account successfully', async () => {
      await supertest(app).delete(`/api/accounts/${account_id}`).expect(http.StatusCodes.OK);
    });

    test('Should return NOT_FOUND when delete account not found', async () => {
      await supertest(app).delete(`/api/accounts/${account_id}`).expect(http.StatusCodes.NOT_FOUND);
    });

    test('Should return NOT_FOUND when find account by id not found', async () => {
      await supertest(app).get('/api/accounts/-5').expect(http.StatusCodes.NOT_FOUND);
    });
  });
