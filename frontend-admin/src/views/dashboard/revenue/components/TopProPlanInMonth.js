...
import { fetchGetTopProPlansInMonth } from '../../../../services/DashboardService';
...
const TopProPlanInMonth = ({ selectedYear, selectedMonth }) => {
  const [topProPlans, setTopProPlans] = React.useState([]);
  const getTopProPlans = async () => {
    try {
      const response = await fetchGetTopProPlansInMonth(selectedYear, selectedMonth);
  ...
  return (
    <DashboardCard title={`Top doanh thu gói Pro tháng ${selectedMonth} năm ${selectedYear}`}>
...
export default TopProPlanInMonth;
